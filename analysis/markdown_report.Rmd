...
### Descriptive Stats - SBP, Ambulatory SBC

Summary stats under 80 BP readings. Includes last systolic blood pressure (SBP), ambulatory sbp (narrow definition), and ambulatory sbp (wide definition)

```{r under80summary}

data %>% 
  filter(age < 80) %>% 
  select(last_systolic_bp_measure,
         last_ambulatory_systolic_bp_measure,
         last_ambulatory_systolic_bp_measure_narrow) %>% 
  Hmisc::describe()
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_135_broad, last_amb_bp_cutoff_140_broad)
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_135_narrow, last_amb_bp_cutoff_140_narrow)
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_140_broad, CKD_code)
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_140_broad, CVD_code)
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_140_broad, T1D_code)
...
data %>% 
  filter(age < 80) %>% 
  tabyl(last_amb_bp_cutoff_140_broad, T2D_code)
...
data %>% 
  filter(age < 80,
         CKD_code == 1) %>% 
  tabyl(last_amb_bp_cutoff_140_narrow, last_amb_bp_cutoff_135_narrow)
...
Summary stats 80 and over. Includes Last systolic blood pressure (SBP), ambulatory sbp (narrow definition), and ambulatory sbp (wide definition)

```{r baseover80}

data %>% 
  filter(age >= 80) %>% 
  select(last_systolic_bp_measure, last_ambulatory_systolic_bp_measure, last_ambulatory_systolic_bp_measure_narrow) %>% 
  Hmisc::describe()
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_broad, last_amb_bp_cutoff_145_broad)
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_narrow, last_amb_bp_cutoff_145_narrow)
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_broad, CKD_code)
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_broad, CVD_code)
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_broad, T1D_code)
...
data %>% 
  filter(age >= 80) %>% 
  tabyl(last_amb_bp_cutoff_150_broad, T2D_code)
